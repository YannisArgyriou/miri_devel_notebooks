...
def mmrs_abtov2v3(a,b,v2,v3,channel,cdpdir=None,xan=None,yan=None):
    ...
    case channel of
    dic = {'1A': reffile='MIRI_FM_MIRIFUSHORT_12SHORT_DISTORTION_06.04.00.fits',
    ...
    convtable=fits.open(reffile)['albe_to_XANYAN'].data
    # Determine which rows we need
    v2index=np.where(strcompress(convtable.(0),/remove_all) == 'T_CH'+channel+'_XAN')
    v3index=np.where(strcompress(convtable.(0),/remove_all) == 'T_CH'+channel+'_YAN')
    if ((v2index < 0)or(v3index < 0)): raise KeyError, 'Neither v2 or v3 should be less than zero.'
