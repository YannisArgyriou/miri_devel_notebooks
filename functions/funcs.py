...
def get_cdps(band,cdpDir,output='img'):
    ...
    fringe_file = os.path.join(cdpDir,\
      'MIRI_FM_%s_%s_FRINGE_7B.02.06.fits' % (detnick,fringe_subchan_name))
    ...
    photom_file = os.path.join(cdpDir,\
      'MIRI_FM_%s_%s%s_PHOTOM_7B.03.02.fits' % (detnick,photom_subchan_name,subchan_names[band[1]]))
...
def fit_emission_lines(x,y,peaks,fit_func='skewed_voight',sigma0=3.5,f0=0.5,a0=0.1):
...
def buriedelectrode_transmission(workDir,wav,concentration='5e14'):
...
def sort_lists(Y,X):
    ...
    """
    # answer:
    return np.sort(X),[x for _,x in sorted(zip(X,Y))]
